...
'''
Author: Kun
Date: 2022-01-24 19:59:29
LastEditTime: 2022-01-24 20:05:19
LastEditors: Kun
Description: 
FilePath: /projects/my_open_projects/GANs-implementation/image_transfer/stargan/stargan.py
'''
...
parser.add_argument("--residual_blocks", type=int, default=6,
...
if opt.epoch != 0:
    ...
    generator.load_state_dict(torch.load(
        "saved_models/generator_%d.pth" % opt.epoch))
    discriminator.load_state_dict(torch.load(
        "saved_models/discriminator_%d.pth" % opt.epoch))
else:
...
def sample_images(batches_done):
    ...
    for i in range(10):
        ...
        for sample_i, changes in enumerate(label_changes):
            for col, val in changes:
                labels[sample_i, col] = 1 - \
                    labels[sample_i, col] if val == -1 else val
    ...
    save_image(img_samples.view(1, *img_samples.shape),
               "images/%s.png" % batches_done, normalize=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        sampled_c = Variable(
            Tensor(np.random.randint(0, 2, (imgs.size(0), c_dim))))
        ...
        # Adversarial loss
        loss_D_adv = -torch.mean(real_validity) + \
